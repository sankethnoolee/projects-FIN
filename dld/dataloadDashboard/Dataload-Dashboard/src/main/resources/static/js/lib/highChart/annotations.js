...
function defatultMainOptions(){
	var buttons = [],
		...
		steps = [ utils.getRect, utils.getText],
		stops = [ utils.getRectAndUpdate, utils.showInput];
...
function attachEvents(chart) {
	function drag(e) {
		...
		if(chart.annotations.options.buttons[selected]!=undefined){
	...
	function drop(e) {
			...
			if(chart.drawAnnotation){
				...
				if(chart.annotations.buttons[chart.annotations.selected]!=undefined){
												self = chart.annotations.buttons[chart.annotations.selected][0];
