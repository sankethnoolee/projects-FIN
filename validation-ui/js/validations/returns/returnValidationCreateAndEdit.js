    ...
    $("#date-picker-id").datepicker({
    	...
    	if($("#date-picker-id").val()!=""){
            ...
            if (isEdit == false) {
                ...
                $('#returnNameSection').append("<option selected disabled>Select Option</option>");
            }
...
function populateReturnValidationData(effectiveDateOnBar) {
    ...
    for (var i = 0; i < editValidationArrayLen; i++) {
    	if (moment(editValidationArray[0].data[i]["effectiveDate"]).format(momentDateFormat) == effectiveDateOnBar) {
            ...
            $('#returnNameId').val(validationArray["validationReturns"].trim());
            ...
            $('#returnNameSection').val(validationArray["validationSection"]);
...
function validationJsonCreation() {
    ...
    if (validationObj["validationType"] == "Optional") {
        if ($('#everyOccurenceDropDown').val() == "Every Occurence")
            validationObj["isCommentAtValidationLevel"] = "N";
        else
            validationObj["isCommentAtValidationLevel"] = "Y";
    } else {
        validationObj["isCommentAtValidationLevel"] = "";
    }
    ...
    validationObj["validationReturns"] = $("#returnNameId").val();
    validationObj["validationSection"] = $("#returnNameSection").val()==null?-9999:$("#returnNameSection").val();
...
function appendOptionsCreateAndEdit(data, id) {
	...
	for (var i = 0; i < dataLen; i++) {
		if (id == "everyOccurenceDropDown" || id == "statusId" || id == "groupReturnId") {
			if (i == 0 && id != "groupReturnId") {
...
function checkMandatoryFieldForValidation() {
    ...
    if ($('#returnNameId').val() == "" || $('#returnNameId').val() == null || $('#returnNameId').val() == undefined) {
        ...
        $('#returnNameId').siblings('.btn-group').after("<p class='vs-alert-text'>Return name is mandatory</p>");
    }
...
function clearInpBoxAndReloadData(ind, elementId,parentId) {
	if(ind=="returnData"){
		 expressionObj = {};
		    ...
		    var fragment = document.createDocumentFragment();
		    var parentElement = document.createElement('div');
		    parentElement.setAttribute('class', 'parent-div');
		    var mainElement = document.createElement('div');
		    mainElement.setAttribute('class', 'contentDiv');
		    mainElement.setAttribute('ondblclick', 'addValueIntoInputBoxOfReftbl(this,"level-1")');
		    mainElement.setAttribute('level', 'level-1');
		    mainElement.textContent = "REFTBL";
		    mainElement.setAttribute('value', "REFTBL");
		    var childElement = document.createElement('div');
		    childElement.setAttribute('class', 'vs-tooltip icon-info');
		    var iElement = document.createElement('i');
		    iElement.setAttribute('class', 'icon-small icon-info-solid');
		    ...
		    var innerSpanElement = document.createElement('span');
		    innerSpanElement.setAttribute('class', 'vs-baseline-medium-primary');
		    innerSpanElement.textContent = "Reference Data Entities";
		    ...
		    parentElement1.setAttribute('class', 'parent-div');
		    var mainElement1 = document.createElement('div');
		    mainElement1.setAttribute('class', 'contentDiv');
		    mainElement1.setAttribute('ondblclick', 'addValueIntoInputBox(this,"level-1")');
		    mainElement1.setAttribute('level', 'level-1');
		    mainElement1.textContent = "RTN";
		    mainElement1.setAttribute('value', "RTN");
		    var childElement1 = document.createElement('div');
		    childElement1.setAttribute('class', 'vs-tooltip icon-info');
		    var iElement1 = document.createElement('i');
		    iElement1.setAttribute('class', 'icon-small icon-info-solid');
		    var spanElement1 = document.createElement('span');
		    spanElement1.setAttribute('class', 'vs-tooltiptext-left');
		    spanElement1.setAttribute('style', 'width:max-content');
		    var innerSpanElement1 = document.createElement('span');
		    innerSpanElement1.setAttribute('class', 'vs-baseline-medium-primary');
		    innerSpanElement1.textContent = "Returns";
		    spanElement1.appendChild(innerSpanElement1);
		    childElement1.appendChild(iElement1);
		    childElement1.appendChild(spanElement1);


		    parentElement1.appendChild(mainElement1);
		    parentElement1.appendChild(childElement1);
		    fragment1.appendChild(parentElement1);
		    $("#expression-data").append(fragment1);
...
function resetMandatoryHighLight(){
 ...
 // ENT module functions
     ...
     function getEntitySubjectArea() {
		$.ajax({
			url: 'getentitysubjectareaforautosuggestion.htm',
			type: 'GET',
			dataType: 'json',
			...
			success: function(response) {
				$('#expression-data .main-parent').remove();
				dEntity["subjectArea"] = response.success.model.subjectAreas;
				var returns = response.success.model.subjectAreas;
				var fragment = document.createDocumentFragment();
				returns.forEach(function(val) {
					var parentElement = document.createElement('div');
					parentElement.setAttribute('class', 'main-parent');
	...
	/* 			response = response.success.model; */
     ...
     function getEntityTableColumnName(entityUuid) {
		$.ajax({
			url: 'getentitytablecolumnnameforautosuggestion.htm',
			type: 'GET',
			dataType: 'json',
			async:false,
			data: {
			...
			success: function(response) {
				$('#expression-data .main-parent').remove();
				dEntity["entityColumnName"] = response.success.model;
