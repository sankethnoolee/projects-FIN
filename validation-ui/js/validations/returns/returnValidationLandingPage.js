...
$(document).ready(function () {
    ...
    $("#uploadForm").submit(function(e) {
        ...
        $.ajax({
            url: 'masseditcheckcreationv2.htm',
            ...
            success: function(response) {
                ...
                if (response.returnStatus == 'VALIDATION_FAILED') {
                    $('#hiddenDownloader').attr('src', 'downloadValidatedFileForMassEditCheckCreationv2.htm');
    ...
    $("#validationListKendo").kendoGrid({
        dataSource: {
            ...
            transport: {
                read: function(options) {
                    ...
                    gridFilter.gridDetails.sortOn = "validationReturns~~asc";
                    gridFilter.gridDetails.validationCode = "";
                    gridFilter.gridDetails.validationName = "";
                    gridFilter.gridDetails.validationSection = "";
                    ...
                    if (options.data.filter != undefined) {
                        for (var i = 0; i < options.data.filter.filters.length; i++) {
                            if (options.data.filter.filters[i].field != undefined) {
                                ...
                                if (options.data.filter.filters[i].field == "validationSection") {
                                    gridFilter.gridDetails.validationSection = options.data.filter.filters[i].value;
                                }
                            }
                        }
                    }
                    $.ajax({
                        type: 'POST',
                        dataType: 'json',
                        url: 'getdataforvalidationgrid.htm',
                        data: {
        ...
        change: function(options) {
          ...
          //  if ($(this.select().children()[5+$('.k-grouping-header').children().length]).text().toLowerCase() == "active") {
            	 $(".overlay").css("display", "block");
            	    $.ajax({
            	        url: 'loadreturnvalidationeditpage.htm',
            	        type: 'POST',
            	        dataType: 'json',
            	        data: {
            	        	validationReturns: selectedItem["validationReturns"],
            	        	validationCode:selectedItem["validationCode"],
            	        	validationSection:selectedItem["validationSection"],
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success == true) {
                $('#hiddenDownloader').attr('src', 'exporttoexcelhidden.htm');
...
function createFilterJson(ind) {
    ...
    if (ind == "export") {
        ...
        mainFilter["validationReturns"] = exportDefDetails["return-exportDef-dropdown"];
        ...
        mainFilter["validationReturns"] = $('#return-select-id option:selected').text() == "All" ? "" : $('#return-select-id option:selected').text();
    ...
    gridFilter["sortOn"] = "validationReturns~~asc";
...
function getMainFilterData(ind) {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (ind == "export") {
                appendOptions(response.response.validationReturn, "return-exportDef-dropdown", 'export');
                ...
                appendOptions(response.response.validationReturn, "return-select-id");
...
function redirectToValidationCreationPage() {
	...
	$('#validation-msg').hide();
    $.ajax({
        url: 'loadreturnvalidationcreatepage.htm',
        type: 'POST',
        dataType: 'json',
        data: {
        },
...
function exportTemplate() {
    $(".overlay").css("display", "block");
    $('#hiddenDownloader').attr('src', 'downloadtemplateforeditcheckv2.htm');
    $(".overlay").css("display", "none");
}
