...
public class ExpressionTokenizer {
    private static final Set<String> FUNCTIONS = new HashSet<>(Arrays.asList("FOREACH", "PERIOD", "VLOOKUP", "SUMIF",
            "STRING", "EQ", "NEQ", "FILTER", "MAXIF", "MINIF", "COUNTIF", "SUM", "MAX", "MIN", "COUNT", "OFFSET", "YEARIN",
            "LEN", "LOWER", "UPPER", "SUBSTR","CONCAT","AND","OR","ISNOTEMPTY","ISEMPTY","ABS","PERIOD", "ROUND", "AVG",
            "CONVERT", "REGEX", "TODATE", "SOM", "EOM", "SOY", "EOY", "SOFY", "EOFY", "DATEPART", "DATEDIFF", "DCOUNT",
            "UNIQUE"));
    ...
    private Token consumeString(final CharacterStream tokenStream) {
        ...
        while (tokenStream.hasNext()) {
            if (Character.isAlphabetic(tokenStream.peek()) ||
                    SPECIAL_CHAR.contains(tokenStream.peek() + "") || Character.isDigit(tokenStream.peek()) || ' ' == (tokenStream.peek())) {
